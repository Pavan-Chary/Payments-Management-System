  ...
  <div class="min-h-screen bg-slate-50 flex flex-col items-center py-12 px-4 font-sans">
    <div class="w-full max-w-xl bg-white shadow-md rounded-2xl border border-slate-200 overflow-hidden">
      ...
      <div class="bg-slate-50 px-8 py-10 border-b border-slate-100 text-center">
        <h1 class="text-2xl font-black text-slate-900 tracking-tight">Payment Registration</h1>
      </div>
      ...
      <div class="p-8 md:p-12">
        ...
        <form @submit.prevent="handleFormSubmit()" class="space-y-6">
          ...
          <div class="relative">
            <label for="transactionId" class="block text-xs font-black uppercase text-slate-900 mb-2 ml-1">Transaction
              ID</label>
            <input required id="transactionId" type="text" v-model="form.transactionId" placeholder="e.g. TXN9901"
            ...
            <div v-if="formError.transactionId" class="text-rose-700 text-[10px] font-black mt-1 ml-1 uppercase italic">
              {{ formError.transactionId }}</div>
          </div>

          <div class="relative">
            <label for="useEmail" class="block text-xs font-black uppercase text-slate-900 mb-2 ml-1">User Email
              ID</label>
            <input required id="useEmail" type="text" v-model="form.userEmail" placeholder="customer@domain.com"
              class="w-full px-4 py-3 bg-white border-2 border-slate-100 rounded-xl outline-none focus:border-slate-900 font-bold text-slate-900 text-sm"
              :class="{ 'border-rose-500': formError.userEmail }">
            <div v-if="formError.userEmail" class="text-rose-700 text-[10px] font-black mt-1 ml-1 uppercase italic">{{
              formError.userEmail }}</div>
          </div>

          <div class="relative">
            <label for="amount" class="block text-xs font-black uppercase text-slate-900 mb-2 ml-1">Amount</label>
            <input required id="amount" type="number" v-model="form.amount"
              class="w-full px-4 py-3 bg-white border-2 border-slate-100 rounded-xl outline-none focus:border-slate-900 font-bold text-slate-900 text-sm"
              :class="{ 'border-rose-500': formError.amount }">
            <div v-if="formError.amount" class="text-rose-700 text-[10px] font-black mt-1 ml-1 uppercase italic">{{
              formError.amount }}</div>
          </div>

          <div class="relative">
            <label for="paymentMethod" class="block text-xs font-black uppercase text-slate-900 mb-2 ml-1">Payment
              Method</label>
            <select v-model="form.category" id="paymentMethod" name="paymentMethod"
